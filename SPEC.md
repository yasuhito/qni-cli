...
MVP では、単一量子ビットゲート `H` / `X` / `Y` / `Z` / `S` / `S†` / `T` / `T†` / `√X` / `P(angle)` / `Rx(angle)` / `Ry(angle)` / `Rz(angle)` と、2 つの対象量子ビットを持つ `SWAP` を任意の `step` と `qubit` に追加する `add` コマンド、指定したセルのゲートを取得する `gate` コマンド、指定したセルのゲート操作を削除する `rm` コマンド、角度変数を管理する `variable` コマンド、状態ベクトルを表示する `run` コマンド、Pauli 文字列の期待値を表示する `expect` コマンド、現在の回路ファイルを削除する `clear` コマンドを対象にする。単一量子ビットゲートには `--control` を付けた制御付き追加も含む。

## 実装言語

- 実装言語は Ruby とする。
- 2026-03-10 時点の最新安定版である Ruby 4.0.1 を開発対象とする。
...
- `"P(<angle>)"`: `../qni` に合わせたパラメータ付き位相ゲート。例: `"P(π/3)"`, `"P(theta)"`。
...
- `"Swap"`: SWAP ゲート。必ず同じステップに 2 個 1 組で置く。
- `"•"`: 制御記号。制御付きゲートの制御量子ビットを表す。
- それ以外の文字列や値は将来の拡張用として読み込み時に許容してよいが、MVP の `add` コマンドは `1` かどうかだけを見て空き判定する。

### CNOT / 制御ゲート表現

- `../qni` の `cols` JSON に合わせ、CNOT は同じステップの同じ列に制御記号 `"•"` と対象ゲート `"X"` を置いて表す。
- 例として、制御量子ビット `0`、対象量子ビット `1` の CNOT は `["•", "X"]` とする。
- 制御量子ビット `1`、対象量子ビット `0` の CNOT は `["X", "•"]` とする。
- 多重制御は `../qni` と同様に `"•"` を複数並べ、対象側には実際のゲート文字列を置く。
...
- `../qni` の `cols` JSON に合わせ、SWAP は同じステップの同じ列に `"Swap"` を 2 個置いて表す。
- 例として、対象量子ビット `0` と `1` の SWAP は `["Swap", "Swap"]` とする。
- 対象量子ビットは必ず 2 つで、1 個や 3 個以上の `"Swap"` は無効とする。
...
- `--step <step>`: 配置先、取得対象、または削除対象のステップ番号。0 始まりの整数。
- `--qubit <qubit>`: 配置先、取得対象、または削除対象の量子ビット番号。0 始まりの整数。`add SWAP` の場合は `0,1` のようにカンマ区切りで 2 つ指定する。
- `--control <control>`: 制御量子ビット番号。カンマ区切りで複数指定できる。`X` と組み合わせた場合は CNOT / 多重制御 X を表す。
- `<angled_gate>`: `P`, `Rx`, `Ry`, `Rz` のいずれか。
- `--angle <angle>`: `P`, `Rx`, `Ry`, `Rz` に必須の角度。`π/3`, `pi/3`, `3*pi/4`, `0.5` のような具体角度か、`theta` のような ASCII 識別子を受け付ける。具体角度は保存時に `P(π/3)`, `Rx(π/2)` のような正規形に正規化し、変数は `Ry(theta)` のようにそのまま保存する。
...
`qni add <gate> --control ...` は、`--control` で指定した量子ビットのセルがすべて `1` であることも確認し、同じ列に `"•"` を配置してから対象側に対象ゲートを置く。`P`, `Rx`, `Ry`, `Rz` の場合は対象側に角度付きの正規形を置く。

`qni add SWAP --qubit 0,1 ...` は、2 つの対象量子ビットが互いに異なり、対象セルがどちらも `1` であることを確認してから、同じ列に `"Swap"` を 2 個配置する。
...
`qni rm --step <step> --qubit <qubit>` は、既存の `./circuit.json` を読み込み、`cols[step][qubit]` のセルを削除する。単一量子ビットゲートは指定セルだけを `1` に戻す。制御付きゲートは制御側または対象側のどちらを指定しても、同じ操作の制御記号 `"•"` と対象ゲートをすべて `1` に戻す。`SWAP` は片方の `"Swap"` セルを指定しても 2 つの `"Swap"` セルをどちらも `1` に戻す。指定セルが存在しない場合、指定セルが `1` の場合、または `./circuit.json` が存在しない場合は失敗する。削除後は回路自動圧縮を適用する。
...
`qubit` が現在の最終量子ビットを超えていてもエラーにはしない。
...
追加または削除後に回路先頭の量子ビットがすべてのステップで `1` の場合、その空の量子ビットは自動的に削除する。削除後に回路末尾の量子ビットがすべてのステップで `1` の場合も、その空の量子ビットは自動的に削除する。初期状態が明示されている場合は、初期状態の量子ビット数を下回る圧縮はしない。
...
この圧縮は先頭または末尾の空の量子ビットに適用し、途中の空の量子ビットは残す。
...
- 制御付き列は `"•"` を制御として、非 `1` / 非 `"•"` の 1 セルを対象として解釈する
...
- 終了コード: `0`
- 標準出力に回路を 1 量子ビット 1 行のアスキーアートで表示する
...
- 書き込みは同じファイルパスへの更新とする。
- 中途半端な破損を避けるため、実装では一時ファイルに書いてから `rename` するアトミック書き込みを前提とする。
- 出力 JSON は 2 スペースインデントと末尾改行を標準とする。
...
### 例 3: `circuit.json` がない状態で位相ゲートを追加
