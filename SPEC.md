...
MVP では、単一量子ビットゲート `H` / `X` / `Y` / `Z` / `S` / `T` を任意の `step` と `qubit` に追加する `add` コマンドと、状態ベクトルを表示する `run` コマンドを対象にする。`X` に `--control` を付けた制御付き追加も対象に含む。
...
```bash
qni add X --control <control> --step <step> --qubit <qubit>
```

### 引数

- `<gate>`: 追加するゲート種別。MVP では `H` と `X` と `Y` と `Z` と `S` と `T` を許可。
...
`qni add X --control ...` は、control に指定した qubit のセルがすべて `1` であることも確認し、同じ列に `"•"` を配置してから target 側の `"X"` を置く。
