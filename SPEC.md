...
MVP では、単一量子ビットゲート `H` を任意の `step` と `qubit` に追加する `add` コマンドと、状態ベクトルを表示する `run` コマンドを対象にする。
...
```bash
qni add H --step <step> --qubit <qubit>
```

### 引数

- `H`: 追加するゲート種別。MVP では `H` のみ許可。
...
6. 回路が既存ファイル由来の場合、`qubit` が `0 <= qubit < qubits` を満たすことを検証する。
