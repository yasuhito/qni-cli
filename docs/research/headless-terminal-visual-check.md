...
脚本は `npm run check` には含めない。tmux 内では端末問い合わせを行わないため、出力された PNG を読み、起動画面の拡張一覧に `qni-math` があることと、入力欄の下に版、`path: text`、`reason: 環境変数 TMUX` が出ることを確認する。

本文の画像経路は、暗い・明るいテーマを Ghostty と Kitty で撮影する。

```bash
scripts/dev/headless_qni_math_images.sh ghostty dark /tmp/qni-math-ghostty-dark.png auto
scripts/dev/headless_qni_math_images.sh ghostty light /tmp/qni-math-ghostty-light.png auto
scripts/dev/headless_qni_math_images.sh kitty dark /tmp/qni-math-kitty-dark.png auto
...
```bash
scripts/dev/headless_qni_math_images.sh ghostty dark /tmp/qni-math-ghostty-text.png text
```
