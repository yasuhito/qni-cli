...
まず `--shots` で回路を初期状態から独立に複数回実行します。通常出力は、回路内の名前付き測定と名前なし測定を列にした共同分布です。

```bash
qni run --shots 100
```

次に `--seed` で疑似乱数を固定すると、同じ回路、測定ショット数、シード値から同じ共同分布を再現できます。シード値を省略した場合は `Math.random` による固定されていない乱数を使います。測定ショット数は正の整数、シード値は 0 以上 4294967295 以下の整数です。

```bash
qni run --shots 100 --seed 42
```

機械処理には `--json` を加えます。JSON は `shots`、使用したシード値を示す `seed`、`classicalBits`、結果ごとの `values` と `count` を返します。シード値を省略した場合の `seed` は `null` です。`values` は文字列のビット順ではなく、`classicalBits` の各名前をキーにして値を参照できます。

```bash
qni run --shots 100 --seed 42 --json
```

名前なし測定の列名は量子ビットに対応する `qN` です。同じ列名が一つの回路内に再登場した場合は、2回目から `#2`、`#3` を付け、すべての測定を共同分布に残します。`--shots`、`--seed`、`--json` は測定を含む回路で使います。
