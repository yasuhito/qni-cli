...
> **エージェント作業者向け:** 必須: サブエージェントが利用できる場合は superpowers:subagent-driven-development を、利用できない場合は superpowers:executing-plans を使ってこの計画を実装する。ステップの追跡にはチェックボックス (`- [ ]`) 構文を使う。
