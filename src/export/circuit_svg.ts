...
class SvgStep {
  ...
  get controlledTarget(): Placement | undefined {
    ...
    const targets = this.operations.filter(
      ({ operation }) => operation.kind === "gate"
    );
    if (targets.length !== 1) {
      throw new Error(
        `unsupported controlled step: ${JSON.stringify(this.operations)}`
      );
    }
    return targets[0];
  }
  ...
  validate(): void {
    ...
    if (
      this.swaps.length !== 2 ||
      this.operations.some(({ operation }) => operation.kind === "gate")
    ) {
      throw new Error(
        `unsupported swap step: ${JSON.stringify(this.operations)}`
      );
    }
    if (
      new Set(this.swaps.map(({ operation }) => operation.classicalCondition))
        .size !== 1
    ) {
...
export class CircuitSvg {
  ...
  constructor(circuit: CircuitData, options: CircuitSvgOptions) {
    ...
    const captionWidth =
      this.caption === ""
        ? 0
        : [...this.caption].length * this.captionSizeSvg + MARGIN * 2;
    this.width = Math.ceil(Math.max(layout.width, captionWidth));
    this.height =
      Math.max(ROW_HEIGHT, this.circuit.qubits * ROW_HEIGHT) +
      this.captionSpace;
  }
  ...
  render(): string {
    const lines = [
      ...
      ...this.steps.flatMap((step, index) =>
        this.renderStep(step, this.columnCenters[index])
      ),
      ...this.bottomCaption,
      "</svg>",
    ];
  ...
  private get bottomCaption(): string[] {
    return this.caption !== "" && this.captionPosition === "bottom"
      ? [
          this.captionText(
            "bottom",
            this.circuitHeight + MARGIN + this.captionSizeSvg
          ),
        ]
      : [];
  }
  ...
  private get captionSpace(): number {
    return this.caption === ""
      ? 0
      : Math.ceil(this.captionSizeSvg * 1.3) + MARGIN * 2;
  }
  ...
  private renderControlLines(
    ...
    return [
      ...
      ...step.controls.map(
        (qubit) =>
          `<circle class="control-dot" data-operation="control" data-step="${step.step}" data-qubit="${qubit}" data-target-qubit="${targetQubit}" cx="${x}" cy="${this.y(qubit)}" r="5"/>`
      ),
    ];
  }

  private renderControlledGate(
    step: SvgStep,
    x: number,
    target: Placement
  ): string[] {
    const lines = this.renderControlLines(
      step,
      x,
      [...step.controls, target.qubit],
      target.qubit
    );
  ...
  private renderStep(step: SvgStep, x: number): string[] {
    ...
    const controlledLines =
      target === undefined ? [] : this.renderControlledGate(step, x, target);
  ...
  private renderSwap(step: SvgStep, x: number): string[] {
    const [first, second] = [...step.swaps].sort(
      (left, right) => left.qubit - right.qubit
    );
    const connectedQubits = [...step.controls, first.qubit, second.qubit];
    const lines =
      step.controls.length > 0
        ? this.renderControlLines(
            step,
            x,
            connectedQubits,
            nearestSwapQubit(step.controls[0], first.qubit, second.qubit)
          )
        : [
    ...
    return [
      ...
      ...(condition === undefined
        ...
        : [
            this.text(
              x + 10,
              this.y(first.qubit) - 11,
              `<${condition}`,
              undefined,
              "annotation"
            ),
          ]),
      ...step.measurements.flatMap((placement) =>
        this.renderMeasurement(step.step, placement, x)
      ),
    ];
  }

  private renderSwapMark(
    step: number,
    qubit: number,
    pairQubit: number,
    x: number
  ...
  private renderTarget(
    ...
    return [
      ...
      ...(condition === undefined
        ? []
        : [
            this.text(x + 13, y - 11, `<${condition}`, undefined, "annotation"),
          ]),
      "</g>",
    ];
  }
  ...
  private y(qubit: number): number {
    return (
      ROW_HEIGHT / 2 +
      qubit * ROW_HEIGHT +
      (this.caption !== "" && this.captionPosition === "top"
        ? this.captionSpace
        : 0)
    );
  }
...
function displayLabel(operation: ParsedCircuitOperation): string {
  if (operation.kind === "measurement") {
    return operation.measurementName === undefined
      ? operation.symbol
      : `${operation.symbol}>${operation.measurementName}`;
  }

  const symbol = operation.symbol === "X^½" ? "√X" : operation.symbol;
  return operation.classicalCondition === undefined
    ? symbol
    : `${symbol}<${operation.classicalCondition}`;
}
...
function nearestSwapQubit(
  control: number,
  first: number,
  second: number
): number {
  return Math.abs(first - control) <= Math.abs(second - control)
    ? first
    : second;
}
