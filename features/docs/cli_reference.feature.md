...
## Scenario: CLI リファレンスは回路図の量子ビットとステップの順序を説明する

- Then リポジトリファイル "docs/cli.md" は "回路図では、上から `q0`、`q1`、… の順に量子ビットを置き、ステップは左から右へ進みます。" を含む

## Scenario: CLI リファレンスは ket の量子ビット順序を説明する

- Then リポジトリファイル "docs/cli.md" は "ket のビット列は左から `q0`、`q1`、… に対応します。" を含む

## Scenario: CLI リファレンスは測定 JSON の列順を説明する

- Then リポジトリファイル "docs/cli.md" は "`classicalBits` は測定操作を実行した順に列名を並べます。" を含む

## Scenario: CLI リファレンスは量子ビット順序を ket の例で示す

- Then リポジトリファイル "docs/cli.md" は "`|10⟩` は `q0=1`、`q1=0` を表します。" を含む
