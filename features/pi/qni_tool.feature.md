...
- Given qni-cli の実行順を記録する偽の Pi ExtensionAPI に qni ツール拡張を登録する
...
## Scenario: LaTeX の結果を pi-formula の公開インターフェースで組版する

- Given pi-formula の公開インターフェースを返す偽の Pi ExtensionAPI に qni ツール拡張を登録する
- When qni ツールに `["run", "--latex"]` を渡す
- Then qni ツールは pi-formula に LaTeX の結果を渡す

## Scenario: pi-formula のテキスト経路では LaTeX の結果を文字列で描く
...
- Given 偽の Pi ExtensionAPI に qni ツール拡張を登録する
- When qni ツール拡張が登録したツール名を確認する
- Then qni ツール拡張は bash ツールを登録していない
...
- Given qni-cli の実行回数を記録する偽の Pi ExtensionAPI に qni ツール拡張を登録する
...
## Scenario: 一括実行の失敗メッセージに成功分と停止位置を示す

- Given 偽の Pi ExtensionAPI に qni ツール拡張を登録する
- When qni ツールで成功、失敗、未実行のコマンドを一括実行する
- Then 一括実行の失敗メッセージは成功分と停止位置を示す

## Scenario: 一括実行は最初の失敗で止まる

- Given 偽の Pi ExtensionAPI に qni ツール拡張を登録する
- When qni ツールで成功、失敗、未実行のコマンドを一括実行する
- Then 一括実行は失敗後のコマンドを実行しない

## Scenario: 一括実行の成功分の変更を残す

- Given 偽の Pi ExtensionAPI に qni ツール拡張を登録する
- When qni ツールで成功、失敗、未実行のコマンドを一括実行する
- Then 一括実行は成功したコマンドの変更を残す
...
- Given 大きな標準出力を返す偽の Pi ExtensionAPI に qni ツール拡張を登録する
...
- Given キャンセル結果を返す偽の Pi ExtensionAPI に qni ツール拡張を登録する
