...
## Scenario: インライン数式の画像配置に画像IDと配置IDを使う
...
## Scenario: インライン数式を本文と釣り合う大きさで描く

- When 単純なインライン数式を端末セルに組版する
- Then インライン数式の内容は端末セル高の 65 パーセント以上になる

## Scenario: 表示数式を本文より大きく描く

- When 単純な表示数式を端末セルに組版する
- Then 表示数式の内容は端末セル高の 75 パーセント以上になる

## Scenario: 1 行では小さくなりすぎるインライン数式をテキスト表示へ戻す

- Given 偽の Pi ExtensionAPI に数式描画拡張を登録する
- When 背の高いインライン数式を画像経路で変換する
- Then 背の高いインライン数式は Markdown のまま残る

## Scenario: テキスト表示へ戻す数式の縮小率を確認する

- When 背の高いインライン数式を端末セルに組版する
- Then インライン数式の縮小率は 65 パーセント未満になる

## Scenario: 小さく見える Pauli 相関をテキスト表示へ戻す
...
- Given `\op` を `\mathrm{#1}` に展開する環境変数で数式描画拡張を起動する
- When `$\op{H}$` を含む本文を画像経路で変換する
- Then 利用者マクロは画像配置になる

## Scenario: 小さくなりすぎる利用者マクロをテキスト表示へ戻す
